...
class Framework
{
	public static function run()
	{
		...
		Framework::initAutoload();
		Framework::initDispatch();
	}
	//定义路径常量(不知道的我不定义 例如...controlle.php我不知道在Admin还是Home下,就不定义了)
	...
	private static function initAutoload()
	{
		...
		spl_autoload_register(function ($class_name) {
			...
			if (in_array($namespace, array('Core', 'Lib'))) {   //命名控件在Core,Lib下
				$path = FRAMEWORK_PATH . $namespace . DS . $class_name . '.class.php';
			} elseif ($namespace == 'Model') {                   //命名空间在Model下
